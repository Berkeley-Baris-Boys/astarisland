...
class QueryPlanner:
    ...
    def next_step(self, aggregator) -> QueryPlanStep:
        ...
        candidates = self._generate_candidates(aggregator, phase)
    ...
    def _generate_candidates(self, aggregator, phase: str) -> list[CandidateWindow]:
        ...
        for seed_index, features in self.seed_features.items():
            ...
            for y in range(0, self.round_detail.map_height - window + 1, stride):
                for x in range(0, self.round_detail.map_width - window + 1, stride):
                    ...
                    score, reason = self._score_window(seed_index, viewport, features, aggregator, phase)
                    candidates.append(CandidateWindow(seed_index=seed_index, viewport=viewport, score=score, reason=reason))
        return candidates

    def _score_window(self, seed_index: int, viewport: Viewport, features: SeedFeatures, aggregator, phase: str) -> tuple[float, str]:
        ...
        if phase == "recon":
            score = 2.0 * coverage_value + 1.8 * frontier + 1.2 * coastal + 1.4 * initial_active + 0.8 * fairness_bonus
            reason = "broad coverage of likely-active region"
        elif phase == "targeted":
            score = 1.6 * frontier + 1.5 * conflict + 1.1 * coastal + 1.3 * entropy + 0.7 * repeat_value + fairness_bonus
